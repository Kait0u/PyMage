...
class InpaintingForm(QDialog):
    def __init__(self, parent: QMainWindow | None = None):
        ...
        self.iter_count = 3
        self.mode_options = ["TELEA", "NS"]
        self.mode = self.mode_options[0]

        title = "Inpainting"
        if parent is not None:
            title = f"{parent.image.name if parent.image is not None else parent.windowTitle()} | Inpainting"
    ...
    def draw_mask(self):
        if self.is_data_valid:
            ...
            yellow = np.zeros_like(self.image.img)
            yellow[:] = YELLOW

            inv = self.orig_image.copy()
            inv.negate()

            self.image.img = (1 - mask) * self.image.img + mask * yellow
    ...
    @staticmethod
    def show_dialog(parent=None) -> tuple[np.ndarray, int, str] | None:
        ipf = InpaintingForm(parent)
        ipf.setModal(True)
        result = ipf.exec()
        return (ipf.mask, ipf.iter_count, ipf.mode) \
